...
async function pollPixStatus(paymentId) {
  if (!paymentId || paymentId !== activePixPaymentId) return;
  try {
    const response = await fetch(`/api/freela-pix-status?payment_id=${encodeURIComponent(paymentId)}&payment_token=${encodeURIComponent(activePixPaymentToken)}`);
    const data = await response.json().catch(() => ({}));
    if (paymentId !== activePixPaymentId) return;
...
async function createPixPayment() {
  ...
  if (!email || !pixEmail.checkValidity()) {
    pixEmail?.focus();
    setCheckoutStatus("Informe seu e-mail para gerar o Pix.");
    return;
  }

  if (pixCreate?.disabled) return;
  stopPixPolling();
  activePixPaymentId = "";
  resetPixVisual();
  ...
  try {
    ...
    if (!response.ok || !data.payment_id || !data.payment_token || !data.qr_code || !data.qr_code_base64) {
      resetPixVisual();
    ...
    activePixPaymentToken = data.payment_token;
