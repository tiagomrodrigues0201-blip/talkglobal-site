...
function pixPaymentToken(paymentId) {
  return crypto.createHmac("sha256", getMercadoPagoToken())
    .update(`freela-pix:${paymentId}`).digest("hex");
}

function hasPixPaymentToken(paymentId, token) {
  if (!getMercadoPagoToken() || !/^[a-f0-9]{64}$/.test(token)) return false;
  return crypto.timingSafeEqual(Buffer.from(pixPaymentToken(paymentId)), Buffer.from(token));
}
...
async function createPixPayment(req, res) {
  ...
  return json(res, 200, {
    ...
    payment_token: pixPaymentToken(pix.paymentId),
    status: pix.status,
    qr_code: pix.qrCode,
...
async function pixStatus(req, res) {
  ...
  if (!hasPixPaymentToken(paymentId, paymentToken)) return json(res, 403, { message: "Acesso ao pagamento Pix inválido." });
  ...
  return json(res, 200, {
    status,
    approved,
    download_url: approved ? `/api/freela-download?mp_payment_id=${encodeURIComponent(paymentId)}&payment_token=${encodeURIComponent(paymentToken)}` : undefined
  });
}
